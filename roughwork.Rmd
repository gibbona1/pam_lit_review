...
p <- df_tools |>
  filter(Automation.Tools != "Other Methods") |>
      ...
      geom_bar(stat = "count") +  # Fill bars with dodgerblue
...
df |>
  ...
  mutate(Expanded_Years = map(Monitoring.year, function(x) {
    if (stringr::str_detect(x, "-")) {
      range <- stringr::str_split(x, "-")[[1]] |> as.numeric()
      return(as.numeric(seq(range[1], range[2])))
    } else {
      return(c(as.numeric(x)))
    }
  })) |>
  ...
  geom_boxplot() +
  # Customize the plot
  labs(title = "Analysis Distribution by Year",
       x = "Analysis",
       y = "Year") +
