...
p <- df |>
  tidyr::drop_na(Dataset_hours) |> 
  #filter(Year >= 2008) |>
  ggplot(aes(Year, Dataset_hours)) + 
  geom_point() + 
  scale_y_log10() +
  geom_smooth(method="lm", color = "dodgerblue") +
  scale_x_continuous(expand = c(0.01,0.01)) +
...
ecosystem_colors <- c(
  "Tropical Forest" = "#228B22",
  "Marine Aquatic" = "#1E90FF",
  "Various" = "#808080",
...
p <- ggplot() +
  ...
  geom_jitter(data = latlong_df, aes(x = long, y = lat, fill = Ecosystem), colour="black", size = 2, shape=21) +
  # Set coordinate system to map projection
  coord_fixed(ratio = 1, xlim = c(-180, 180), ylim = c(-90, 90), expand = FALSE) +
...
barplot_counts <- function(df, col, col_title=col, prop=FALSE, year=TRUE, decade=FALSE){
  if(decade){
    df <- df |>
  ...
  if(!year){
    ...
    p <- df |>
      ...
      geom_bar(stat = "count", fill = "dodgerblue") +  # Fill bars with dodgerblue
    ...
    if(prop){
      p <- count_data |> 
        ...
        labs(x = "Year", y = "Percentage", fill = col_title) +
...
p
ggsave("images/preprocessing_prop_decade.png", p, width = 240, height = 100, units = "mm")
```
```{r}
count_data <- df_lang |>
